...
export function renderScreen(
  ...
  switch (screen.name) {
    ...
    case 'POI_DETAIL':
      return renderPoiDetail(screen.poi, units)

    case 'POI_ACTIONS':
      return renderPoiActions(screen.poi, screen.actions, screen.cursorIndex)

    case 'NAV_ACTIVE':
      return renderNavActive(screen, units)
 ...
 */
export function renderInPlaceUpdate(
  screen: Screen,
  units: 'imperial' | 'metric' = 'imperial',
): TextContainerUpgrade | null {
  switch (screen.name) {
    case 'POI_ACTIONS':
    ...
    case 'NAV_ACTIVE':
      return new TextContainerUpgrade({
        ...
        content: navBodyText(screen, units),
      })
...
function renderPoiList(
  ...
  displayed.forEach((p, i) => {
    items.push(poiListLine(p, poiBase + i === cursorIndex, units))
  })
...
function poiListLine(p: Poi, isCursor: boolean, units: 'imperial' | 'metric'): string {
  ...
  const cursor = isCursor ? '> ' : '  '
  const distance = formatDistance(p.distanceMiles, units)
  const name = truncate(p.name, 32)
  return `${cursor}${p.categoryIcon} ${name}  ${distance}`
}

function renderPoiDetail(poi: Poi, units: 'imperial' | 'metric'): RebuildPageContainer {
  return new RebuildPageContainer({
    containerTotalNum: 2,
    textObject: [
      new TextContainerProperty({
        ...
        isEventCapture: 0,
        content: detailHeaderText(poi, units),
      }),
      new TextContainerProperty({
        xPosition: 0,
...
function detailHeaderText(poi: Poi, units: 'imperial' | 'metric'): string {
  ...
  const dist = `${formatDistance(poi.distanceMiles, units)} ${bearingToCardinal(poi.bearingDegrees)}`
...
function detailBodyText(poi: Poi): string {
  ...
  if (poi.openingHours !== null) {
    if (poi.isOpenNow === true) {
      parts.push('Open now', '')
    } else if (poi.isOpenNow === false) {
      parts.push('Closed', '')
    } else {
      parts.push(truncate(poi.openingHours, 28), '')
    }
  }
  ...
  parts.push(summary, '', RULE, '', '> Tap for options  ·  2× to go back')
  return parts.join('\n')
}
...
function navBodyText(
  ...
  const lines = [
    `${arrow}  ${formatMeters(remainingMeters, units)}`,
...
function formatDistance(miles: number, units: 'imperial' | 'metric'): string {
  if (units === 'metric') {
...
function formatMeters(m: number, units: 'imperial' | 'metric'): string {
  if (units === 'metric') {
