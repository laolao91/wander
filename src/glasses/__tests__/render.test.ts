...
// ─── POI_DETAIL — opening hours ──────────────────────────────────────────

function makePoiWithHours(
...
describe('POI_DETAIL opening hours in body', () => {
  it('shows "Open now" when isOpenNow is true', () => {
    const screen = { name: 'POI_DETAIL' as const, poi: makePoiWithHours('Mo-Fr 09:00-18:00', true) }
    const result = renderScreen(screen, 'imperial')
    const body = result.textObject?.find(c => c.containerName === 'detail-body')
    expect(body?.content).toContain('Open now')
  })

  it('shows "Closed" when isOpenNow is false', () => {
    const screen = { name: 'POI_DETAIL' as const, poi: makePoiWithHours('Mo-Fr 09:00-18:00', false) }
    const result = renderScreen(screen, 'imperial')
    const body = result.textObject?.find(c => c.containerName === 'detail-body')
    expect(body?.content).toContain('Closed')
  })

  it('shows raw string when isOpenNow is null but openingHours is present', () => {
    const screen = { name: 'POI_DETAIL' as const, poi: makePoiWithHours('Mo-Fr 9-18', null) }
    const result = renderScreen(screen, 'imperial')
    const body = result.textObject?.find(c => c.containerName === 'detail-body')
    expect(body?.content).toContain('Mo-Fr 9-18')
  })

  it('shows no hours line when openingHours is null', () => {
    const screen = { name: 'POI_DETAIL' as const, poi: makePoiWithHours(null, null) }
    const result = renderScreen(screen, 'imperial')
    const body = result.textObject?.find(c => c.containerName === 'detail-body')
    expect(body?.content).not.toContain('Open')
    expect(body?.content).not.toContain('Closed')
  })
})
...
describe('renderScreen units param — metric formatting', () => {
  function makePoi(distanceMiles: number) {
    return {
      id: 't', name: 'Place', category: 'landmark' as const, categoryIcon: '★',
      lat: 0, lng: 0, distanceMeters: Math.round(distanceMiles * 1609.344),
      distanceMiles, bearingDegrees: 0, walkMinutes: 2,
      wikiTitle: null, wikiSummary: null, websiteUrl: null, source: 'osm' as const,
      openingHours: null, isOpenNow: null,
    }
  }

  it('shows km for distances >= 1km in metric mode', () => {
    const screen = {
      name: 'POI_DETAIL' as const,
      poi: { ...makePoi(1.0), distanceMeters: 1609 },
    }
    const result = renderScreen(screen, 'metric')
    const header = result.textObject?.find(c => c.containerName === 'detail-header')
    expect(header?.content).toContain('km')
  })

  it('shows m for distances < 1km in metric mode', () => {
    const screen = {
      name: 'POI_DETAIL' as const,
      poi: makePoi(0.06),
    }
    const result = renderScreen(screen, 'metric')
    const header = result.textObject?.find(c => c.containerName === 'detail-header')
    expect(header?.content).toMatch(/\d+ m/)
  })
})
