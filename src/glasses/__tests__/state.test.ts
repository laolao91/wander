...
// ─── NAV_ACTIVE step advancement helpers ───────────────────────────────

function makeNavState(opts: {
  ...
  const secondStep: RouteStep = {
    ...
    street: null,
    endPoint: [48.8583, 2.2945],
  }
  const destination: Poi = {
    id: 'dest', name: 'Eiffel Tower', category: 'landmark',
    categoryIcon: '★', lat: 48.8583, lng: 2.2945,
    distanceMeters: 500, distanceMiles: 0.31, bearingDegrees: 0,
    walkMinutes: 6, wikiTitle: null, wikiSummary: null, websiteUrl: null,
    source: 'osm', openingHours: null, isOpenNow: null,
  }
  const steps = opts.steps ?? [defaultStep, secondStep]
  const pos = opts.position !== undefined ? opts.position : { lat: 48.8650, lng: 2.2945 }
  return {
    ...INITIAL_STATE,
    position: pos,
    screen: {
      ...
      route: {
        ...
        steps,
        geometry: [[48.865, 2.295], [48.858, 2.294]] as [number, number][],
        language: 'en',
      },
...
describe('NAV_ACTIVE step advancement and arrival', () => {
  it('sets arrived=true and emits stop-nav-watch when within 20m of destination', () => {
    // Place user ~10m from destination (48.8583, 2.2945)
    const state = makeNavState({ position: { lat: 48.8584, lng: 2.2945 } })
    const { state: next, effects } = reduce(state, {
      type: 'position-updated', lat: 48.8584, lng: 2.2945,
    })
    const screen = next.screen as Extract<typeof next.screen, { name: 'NAV_ACTIVE' }>
    expect(screen.arrived).toBe(true)
    expect(effects).toContainEqual({ type: 'stop-nav-watch' })
  })

  it('advances currentStepIndex when within 25m of step endPoint', () => {
    // step 0 endPoint is [48.8600, 2.2950] — place user exactly on it
    const state = makeNavState({ position: { lat: 48.8650, lng: 2.2945 } })
    const { state: next } = reduce(state, {
      type: 'position-updated', lat: 48.8600, lng: 2.2950,
    })
    const screen = next.screen as Extract<typeof next.screen, { name: 'NAV_ACTIVE' }>
    expect(screen.currentStepIndex).toBe(1)
  })

  it('does not advance past the last step', () => {
    const singleStep: RouteStep = {
      instruction: 'Arrive',
      distanceMeters: 10, durationSeconds: 8,
      maneuverType: 'arrive', street: null,
      endPoint: [48.8584, 2.2945],
    }
    const state = makeNavState({ steps: [singleStep], currentStepIndex: 0 })
    const { state: next } = reduce(state, {
      type: 'position-updated', lat: 48.8584, lng: 2.2945,
    })
    const screen = next.screen as Extract<typeof next.screen, { name: 'NAV_ACTIVE' }>
    // arrives (within 20m of destination) rather than trying to advance to index 1
    expect(screen.arrived).toBe(true)
  })

  it('does not advance when step.endPoint is null', () => {
    const noEndStep: RouteStep = {
      instruction: 'Head north', distanceMeters: 200, durationSeconds: 150,
      maneuverType: 'depart', street: null, endPoint: null,
    }
    const noEndStep2: RouteStep = { ...noEndStep, instruction: 'Turn left' }
    const state = makeNavState({ steps: [noEndStep, noEndStep2], position: { lat: 48.8650, lng: 2.2945 } })
    const { state: next } = reduce(state, {
      type: 'position-updated', lat: 48.8600, lng: 2.2950,
    })
    const screen = next.screen as Extract<typeof next.screen, { name: 'NAV_ACTIVE' }>
  ...
  it('does not advance when far from step endPoint', () => {
    // User is far from step 0 endPoint [48.8600, 2.2950]
    const state = makeNavState({ position: { lat: 48.8650, lng: 2.2945 } })
    const { state: next } = reduce(state, {
      type: 'position-updated', lat: 48.8650, lng: 2.2945,
    })
    const screen = next.screen as Extract<typeof next.screen, { name: 'NAV_ACTIVE' }>
    expect(screen.currentStepIndex).toBe(0)
  })
