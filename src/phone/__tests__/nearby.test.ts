...
describe('nearby-refresh-requested', () => {
  it('transitions to locating and emits request-location', () => {
    ...
    expect(result.effects).toEqual([{ type: 'request-location', manualLocation: null }])
  })
...
describe('location-acquired', () => {
  ...
  it('bakes in current settings at emit time (snapshot)', () => {
    const customState: PhoneState = {
      ...locatingState,
      settings: { radiusMiles: 1.5, enabledCategories: ['museums'], units: 'imperial', sort: 'proximity', maxResults: 20, manualLocation: null, lang: null },
    }
...
describe('nearby events do not disturb Settings state', () => {
  ...
  it('settings change triggers a nearby refresh (sets fetchStatus to locating)', () => {
    ...
    expect(result.effects).toContainEqual({ type: 'request-location', manualLocation: null })
