...
import { haversine, bearing } from './_lib/geo.js'
...
const OpeningHoursParser = _require('opening_hours') as {
  new(
    ...
    optional_conf_parm?: Record<string, unknown>,
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  const limitParam = clamp(
    typeof req.query.limit === 'string' && Number.isFinite(parseInt(req.query.limit, 10))
  ...
  try {
    ...
    const hasMore = offset + limitParam < allMerged.length
    console.log(
      '[poi] result',
      JSON.stringify({
...
out center tags 100;`
  ...
  const results = elements
    .map((el) => {
      ...
      const ohRaw = tags.opening_hours ?? null
      const { openingHours, isOpenNow } = parseOpeningHours(ohRaw)
...
function parseOpeningHours(raw: string | null): {
  ...
  if (!raw) return { openingHours: null, isOpenNow: null }
  try {
    const oh = new OpeningHoursParser(raw, null, { mode: 0 })
    return { openingHours: raw, isOpenNow: oh.getState() }
  } catch {
    return { openingHours: raw, isOpenNow: null }
  }
}
